...
public class Importance {
	...
	public static void calcAttrImp(ClassOrInterfaceDeclaration declaration, List<Attribute> allAttributes, AttributeRepository attribute_Repository)
	{
        ...
        for (int x=1; x<allWriteCount.length;x++)
        {
        	...
        	if (allseeCount_process[x]>MaxRp)
        		MaxRp=allseeCount_process[x];        	
        	if (allWriteCount_process[x]>MaxWp)
        		MaxWp=allWriteCount_process[x];        	
        }   
        ...
        */
        ...
        for (int x=0; x<allAttributes.size();x++)
        {
        	...
        	if (MaxR>0)
        		imp1 += (float) ((allseeCount[x] * 1.0 / MaxR)/5.0); 
        	if (MaxW>0)
        	...
        	allAttributes.get(x).setImp1(imp1);
        	allAttributes.get(x).setImp2(imp2);
